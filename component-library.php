...
class ComponentLibrary
{
    ...
    public function generate_sample_data()
    {
        ...
        // Создаем категории портфолио
        $portfolio_categories = array('Кухни', 'Шкафы', 'Прихожие', 'Гостиные');

        foreach ($portfolio_categories as $cat_name) {
            $term = wp_insert_term($cat_name, 'portfolio-cat');
            if (!is_wp_error($term)) {
                $results['categories']++;
            }
        }

        // Создаем работы портфолио
        $portfolio_names = array(
            ...
            'Шкаф угловой',
            'Прихожая белая',
            'Гостиная скандинавия'
        );

        foreach ($portfolio_names as $index => $name) {
            $portfolio_id = wp_insert_post(array(
                'post_title' => $name,
                'post_content' => 'Описание проекта ' . $name,
                'post_status' => 'publish',
                'post_type' => 'portfolio'
            ));
            ...
            if ($portfolio_id && !is_wp_error($portfolio_id)) {
                ...
                $img1 = $this->sample_images[array_rand($this->sample_images)];
                $img2 = $this->sample_images[array_rand($this->sample_images)];

                update_post_meta($portfolio_id, '_img-1', $img1);
                update_post_meta($portfolio_id, '_img-2', $img2);

                // Добавляем категорию
                $terms = get_terms(array('taxonomy' => 'portfolio-cat', 'hide_empty' => false, 'number' => 1));
                if (!empty($terms)) {
                    wp_set_post_terms($portfolio_id, array($terms[0]->term_id), 'portfolio-cat');
                }

                if (!isset($results['portfolio'])) {
                    $results['portfolio'] = 0;
                }
                $results['portfolio']++;
            }
